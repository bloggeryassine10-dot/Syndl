...
(function () {
    ...
    // ============================================
    // LOAD MOVIE DATA (ASYNC FOR FIREBASE)
    // ============================================
    let movie = null;
    const movieId = getMovieId();

    // Initialize database and wait for Firebase to load
    MoviesDB.init(function () {
        movie = MoviesDB.getById(movieId);

        if (!movie) {
            // Try again after a short delay (Firebase might still be loading)
            setTimeout(() => {
                movie = MoviesDB.getById(movieId);
                if (!movie) {
